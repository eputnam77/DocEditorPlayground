...
export default function PluginManager({
  ...
  return (
    <div className="relative inline-block text-left">
      <button
        type="button"
        className="flex items-center gap-1 rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-900 hover:bg-slate-100 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800"
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-2 w-56 space-y-1 rounded-md border border-slate-300 bg-white p-2 shadow-lg dark:border-slate-700 dark:bg-slate-900">
              {plugins.map((p) => (
                <label key={p.name} className="flex items-center gap-2 rounded px-1 py-1 text-sm text-slate-800 hover:bg-slate-100 dark:text-slate-100 dark:hover:bg-slate-800">
                  <input
